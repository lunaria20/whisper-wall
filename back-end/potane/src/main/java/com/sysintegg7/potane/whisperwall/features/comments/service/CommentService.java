package com.sysintegg7.potane.whisperwall.features.comments.service;

import com.sysintegg7.potane.whisperwall.features.comments.dto.CommentRequest;
import com.sysintegg7.potane.whisperwall.features.comments.dto.CommentResponse;
...
public class CommentService {
    ...
    public CommentResponse createComment(Long confessionId, Long userId, CommentRequest request) {
        ...
        if (Boolean.TRUE.equals(user.getIsRestricted())) {
            if (user.getRestrictionEndDate() != null && java.time.LocalDateTime.now().isAfter(user.getRestrictionEndDate())) {
                ...
                throw new UnauthorizedException("You are restricted from commenting. Reason: " + user.getRestrictionReason());
            }
        }
    ...
    public Page<CommentResponse> getConfessionComments(Long confessionId, Pageable pageable) {
        return commentRepository.findByConfessionIdAndIsVisibleTrueOrderByCreatedAtDesc(confessionId, pageable)
